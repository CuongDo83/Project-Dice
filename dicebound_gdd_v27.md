...
- **Owner:** Long Thăng x Cường Đẹp
- **Last updated:** 16/04/2026
- **Purpose of this doc:** Tài liệu mô tả current locked design của prototype hiện tại, gồm implemented baseline và các update đã được khóa đến version 27
- **Intended use:**
  - Design alignment
...
17. Nếu level còn wave chưa spawn, board phải preview trước các vị trí spawn đã schedule của wave kế tiếp:
   - `X` đỏ = vị trí enemy của wave kế tiếp
   - `X` xanh = vị trí mystery cell mới của wave kế tiếp  
18. Ở đầu mỗi `Player turn` thuộc phase `Day`, nếu còn wave đã schedule nhưng chưa spawn thì wave đó spawn ngay, kể cả khi trên board vẫn còn enemy của các wave trước  
...
- Chuyển wave phải readable nhưng áp lực hơn: wave mới phải được gọi theo nhịp `đầu Player turn ban ngày` thay vì chờ giết xong enemy cuối cùng của wave trước, để tiến trình màn đều hơn và không bị chùng vì một enemy sót lại.
...
- Mỗi wave phải có ít nhất `1` mystery cell mới để player có mục tiêu route đầu wave; đồng thời vị trí spawn của wave kế tiếp vẫn phải được preview bằng `X` đỏ / `X` xanh` trước khi wave đó vào ở đầu Day turn.
...
8. Player có thể quay lại bất kỳ stage cũ nào đã clear trong `Rune Trial`
9. Stage cũ đã clear có thể dùng `sweep / auto-win` để claim reward nhanh
   - sweep chỉ áp dụng cho stage cũ đã clear
   - không áp dụng cho stage cao nhất hiện có
...
- **Official daily mode direction:**
  ...
  - stage cũ đã clear có thể quay lại và dùng `sweep / auto-win`
...
- Nếu level còn wave chưa spawn, board phải preview trước vị trí spawn của wave kế tiếp:
  - hiện `X` đỏ cho enemy spawn positions của wave mới
  - hiện `X` xanh cho mystery spawn positions của wave mới
- Ở đầu `Player turn` thuộc phase `Day`, nếu còn wave đã schedule nhưng chưa spawn thì wave mới và mystery mới phải spawn ngay tại các ô đã preview, kể cả khi enemy của wave cũ vẫn còn sống
...
- **Trigger:** Wave đầu spawn khi level bắt đầu; nếu còn wave chưa spawn thì wave kế tiếp vào ở đầu `Player turn` thuộc phase `Day`
...
- **Preview rule:** Nếu wave tiếp theo tồn tại, game phải preview trước các vị trí spawn đã schedule của wave đó trước khi tới đầu `Day` turn nơi wave sẽ spawn
- **Preview marker rule:**
  - `X` đỏ = vị trí enemy của wave kế tiếp
...
- **Spawn rule:** Ở đầu `Player turn` thuộc phase `Day`, nếu còn wave đã schedule nhưng chưa spawn thì enemy mới và mystery mới spawn tại các ô đã preview
...
Khi tới đầu `Player turn` thuộc phase `Day`, wave kế tiếp spawn ngay tại các ô đã preview, kể cả khi trên board vẫn còn enemy của các wave trước. Điều này có nghĩa là overlap giữa wave cũ và wave mới là rule hiện hành; game không xóa enemy cũ chỉ vì wave mới bắt đầu. Preview chỉ cho biết vị trí, không cho biết exact enemy info, để vừa readable hơn vừa vẫn giữ cảm giác may / xui. Toàn bộ mystery cell **chưa ăn** đang còn trên màn sẽ được giữ lại và cộng dồn với mystery của wave mới. Ô preview không chặn player; player vẫn có thể đi qua hoặc đứng trên đó. Current locked overlap outcome của v23 là:
...
- Nếu còn wave tiếp theo, game phải preview cả `X` đỏ cho enemy spawn positions và `X` xanh cho mystery spawn positions ngay khi wave hiện tại kết thúc
- Player phải có `1` full player turn để chuẩn bị sau preview đó
...
- Tăng readability / fairness của chuyển wave bằng 1 lượt báo trước và thêm preview thông tin của mystery mới, đồng thời vẫn giữ tension và anticipation.
...
- **Current cost direction after v27:**
...
- **Rules:**
  ...
  - player có thể quay lại bất kỳ stage cũ nào đã clear
  - stage cũ đã clear có button `sweep / auto-win` để nhận reward
  - sweep chỉ áp dụng cho stage cũ đã clear
  - stage cao nhất hiện có không được sweep
  - sweep tiêu `1` rewarded win
  - sweep reward = manual-clear reward
...
- Giảm friction của daily farm bằng khả năng replay stage cũ và sweep stage cũ
...
- Nếu còn wave chưa spawn và tới đầu `Player turn` thuộc phase `Day` → wave kế tiếp spawn tại các ô đã được preview trước, kể cả khi enemy của các wave trước vẫn còn sống
...
- **Daily tree-resource mode:** `Rune Trial`
  ...
  - old cleared stages can be replayed; sweep is allowed only on older cleared stages, not on the highest cleared stage
- **Current tree-cost direction after v27:**
...
- **Secondary stage structure:** `Rune Trial`
  ...
  - no `Mystery cell`
  - no `Day / Night`
  - any previously cleared older stage may be replayed; sweep is allowed only on older cleared stages, not on the current highest cleared stage
...
- Nếu level dùng fixed waves và còn wave chưa spawn trong main mode, board phải preview đúng `X` đỏ cho enemy spawn positions và `X` xanh cho mystery spawn positions của wave kế tiếp trước lần spawn theo rule hiện hành của main mode
...
- Ở đầu `Player turn` thuộc phase `Day` của main mode, nếu còn wave đã schedule nhưng chưa spawn thì enemy mới và mystery mới phải spawn đúng tại các ô đã preview
...
- `Rune Trial` sweep chỉ được dùng cho stage cũ đã clear, không cho stage cao nhất hiện có, và reward phải bằng manual-clear reward
### 9.2 UI
